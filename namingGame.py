...
class NamingGame(object):
    ...
    def __repr__(self):
        return "NamingGame(actors=" + str(self.actors) + ", maxIterations=" + str(self.maxIterations) + ")"
    ...
    def play(self):
        LOGGER.info("Starting naming game with " + str(self.actors)
                + " fully connected actors, and will play for " + str(self.maxIterations) + " iterations.")
        iteration = 0
        numberOfWords = 3
        statsString = '{"run-' + str(time.time()) + '": [[numberOfWords = ' + str(numberOfWords) + ", iterations = " + str(self.maxIterations) + "],\n"
